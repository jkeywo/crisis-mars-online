...
describe('seating', () => {
  ...
  it('keeps the chair when someone drops', () => {
    ...
    host.submit(host.state.seats.s1, { verb: 'claim-role', payload: { roleId: 'C2' } });
    host.disconnect('s1');
    ...
    expect(host.state.seats.s1.roleId).toBe('C2');
    expect(host.vacantRoles()).toContain('C2');   // ...unless a facilitator says so
  });
  ...
  it('promotes a player who turns out to hold the PIN, and takes their role back', () => {
    ...
    expect(host.vacantRoles()).toContain('C2');
  });

  it('turns away one player more than the roster seats, but never a facilitator', () => {
    // The cap is the *player* roles: the two NPC lanyards are the
    // facilitator's to roleplay, so they are not chairs to fill.
    const host = newHost({ playerCount: 8 });
    for (let i = 0; i < 8; i++) host.identify({ token: `tok-${i}`, name: `p${i}` });
...
describe('the pipeline', () => {
  it('carries a refusal reason back instead of changing anything', () => {
    ...
    const result = host.submit(host.state.seats.s1,
      { verb: 'claim-role', payload: { roleId: 'N1' } });

    expect(result.ok).toBe(false);
    expect(result.reason).toContain('facilitator');
  ...
  it('does not let the reducer clone strand the live seats', () => {
    ...
    host.submit(seat, { verb: 'claim-role', payload: { roleId: 'C2' } });
    expect(host.state.seats.s1).toBe(seat);
    expect(seat.roleId).toBe('C2');
  });
});

describe('projections', () => {
  it('gives a player their own lanyard back and nobody the machinery', () => {
    const host = newHost();
    const alice = host.identify({ token: 'tok-a', name: 'Alice' });
    host.submit(alice, { verb: 'claim-role', payload: { roleId: 'C1' } });

    const seen = host.viewFor(alice);
    expect(seen.brief.personalGoal).toBe(data.roles.roles.C1.private.personalGoal);
    expect(seen.roles.C1.claimedBySeat).toBe('s1');
  ...
  it('gives a facilitator everything', () => {
    ...
    expect(seen.log).toBeInstanceOf(Array);
  });
});
...
describe('saving and restoring', () => {
  ...
  it('restores a short-handed game to the roster it was dealt', () => {
    const host = newHost({ playerCount: 9 });
    const { host: restored, refused } = GameHost.restore({ save: host.save(), data });
    expect(refused).toEqual([]);
    expect(restored.state.rosterCodes).toEqual(data.scaling.rosterAt['9']);
    expect(Object.keys(restored.state.roles).sort())
      .toEqual(Object.keys(host.state.roles).sort());
  });
  ...
  it('names a download so the files sort by when they were taken', () => {
    const host = newHost();
    expect(saveFilename(host.state)).toBe('mars-RAVEN7Z-t00-lobby.json');
  });
...
describe('autosave', () => {
  ...
  it('lists games newest first and skips a corrupt entry', () => {
    ...
    storage.setItem('cmo:save:BROKEN', '{{{');
...
describe('a host that goes away mid-game', () => {
  ...
  it('comes back to the same room code, and everyone reconnects into their seats', async () => {
    ...
    clients[0].manager.send(command('claim-role', { roleId: 'C1' }, 1));
    clients[1].manager.send(command('claim-role', { roleId: 'V1' }, 1));
    await settle();
    expect(host.state.seats.s1.roleId).toBe('C1');
    expect(host.state.seats.s2.roleId).toBe('V1');
    ...
    expect(host.state.seats.s1.roleId).toBe('C1');
    expect(host.state.seats.s2.roleId).toBe('V1');
    const canopy = clients[0].received.filter((m) => m.type === VIEW).at(-1);
    expect(canopy.data.viewer.roleId).toBe('C1');
    expect(canopy.data.brief.personalGoal).toBe(data.roles.roles.C1.private.personalGoal);

    // And the game carries on from where it was.
    clients[0].manager.send(command('claim-role', { roleId: 'C2' }, 2));
    await settle();
    expect(host.state.seats.s1.roleId).toBe('C2');
