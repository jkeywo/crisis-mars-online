...
export class GameHost {
  ...
  static create({ joinCode, seed, data, playerCount, rosterCodes, ...rest }) {
    return new GameHost({
      state: createInitialState({ joinCode, seed, data, playerCount, rosterCodes }),
      data,
      ...rest,
    });
  }
   ...
   */
  static restore({ save, data, rosterCodes, ...rest }) {
    const { state, refused } = replay(save, data, { rosterCodes });
  ...
  identify({ token, name, wantsFacilitator }) {
    ...
    // The two NPC lanyards are facilitator-played, so they are not chairs a
    // player could fill: a "full" game is one with every player role taken.
    const playable = Object.values(this.state.roles).filter((r) => !r.npc).length;
    if (!wantsFacilitator && players.length >= playable) {
      return null;
    }
  ...
  viewFor(seat) {
    return projectView(this.state, this.data, {
      ...
      seatId: seat.id,
      roleId: seat.roleId,
      // The faction is printed on the lanyard, so it is read off the static
      // data rather than kept in state.
      teamId: seat.roleId ? this.data.roles.roles[seat.roleId]?.factionId ?? null : null,
    });
  }
