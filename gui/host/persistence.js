...
export const SAVE_PREFIX = 'cmo:save:';
/**
 ...
 */
export const TURN_PREFIX = 'cmo:turn:';
...
export function saveFilename(state) {
  ...
  return `mars-${state.joinCode}-t${turn}-${state.phase.name}.json`;
}
...
<title>Crisis Mars — ${joinCode}</title>
<style>
  ...
  li { padding: 0.2rem 0; }
  .cm-meta { color: #6b5c48; font-size: 0.85em; }
  .cm-empty { color: #6b5c48; font-style: italic; }
  dt { font-weight: 600; margin-top: 0.7rem; }
  dd { margin: 0; }
</style></head>
<body><h1>Crisis Mars</h1>${body}</body></html>`;
}
