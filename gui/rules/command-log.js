...
export function toSave(state) {
  return {
    ...
    seed: state.seed,
    // The roster rides along too. Which role codes were dealt in is part of
    // the opening position rather than something anybody did, so a
    // short-handed game cannot be rebuilt from the log alone.
    rosterCodes: state.rosterCodes,
    log: state.log,
    seats: state.seats,
