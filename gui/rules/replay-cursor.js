...
export class ReplayCursor {
  /**
   * @param {{joinCode: string, seed: number, log: object[], rosterCodes?: string[]}} save
   * @param {object} data
   * @param {object} [options]
   * @param {number} [options.every]  entries between snapshots
   * @param {string[]} [options.rosterCodes]  only used when the save carries none
   */
  constructor(save, data, { every = CHECKPOINT_EVERY, rosterCodes } = {}) {
    this._save = save;
    this._data = data;
    this._log = save.log ?? [];
    ...
    this._checkpoints = [openingPosition(save, data, { rosterCodes })];
