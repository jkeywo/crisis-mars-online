...
export function admit(state, data, cmd, actor) {
  ...
  //
  // Asked BEFORE the phase-membership check below, and that order is the whole
  // of why the refusal under it reads as English. The four playing phases are
  // named by their own first word, so "that belongs to the maintenance phase,
  // and this is the team phase" is a sentence; the two brackets are not phases
  // anybody plays and are not called this anywhere a player can see — the
  // clock says "Waiting to begin" and "The Aftermath". Reached in the other
  // order, somebody who pressed Recruit soldiers a half-second after time was
  // called was told "this is the epilogue phase", which is the codebase's word
  // and not the game's. Answered here, they are told the game is over.
