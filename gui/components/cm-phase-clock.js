/**
 * <cm-phase-clock> — the turn, the phase, and how long is left.
 *
 ...
 */
const BRACKET_NOTES = {
  lobby: 'Not started. Take a lanyard — the game begins when the facilitator says so.',
...
const PHASE_NOTES = {
  team: 'Talk to your own faction only. Make a plan; brief your allies.',
  negotiation: 'Talk to anyone. Place your action card on a map — it is mandatory.',
  action: 'Players are called to the maps in initiative order to resolve actions.',
};

export class CmPhaseClock extends HTMLElement {
  ...
  connectedCallback() {
    if (!this._built) {
      this._built = true;
      this.innerHTML = `
        <div class="cm-clock-turn"></div>
        <div class="cm-clock-time"><time></time></div>
        <div class="cm-clock-note"></div>`;
    }
    this._render();
    this._ensureTicking();
  }
  ...
  _render() {
    ...
    this.querySelector('.cm-clock-turn').textContent =
    ...
    this.querySelector('.cm-clock-note').textContent = BRACKET_NOTES[phase.name]
      ?? (phase.paused
  ...
  _announce(phase, left) {
    ...
    if (this._step === null) {
      this._step = step;
      this.dispatchEvent(new CustomEvent('cm-time-up', {
    ...
    if (step > this._step) {
      this._step = step;
      this.dispatchEvent(new CustomEvent('cm-overtime', {
        bubbles: true, detail: { overMs: -left },
      }));
    }
  }
...
customElements.define('cm-phase-clock', CmPhaseClock);
